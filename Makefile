...
ifeq ($(PLATFORM),osx)
	...
	PRODUCTION_MACOS_BUNDLE_VERSION := '$(PRODUCTION_MACOS_BUNDLE_VERSION)'
	PRODUCTION_MACOS_CATEGORY := '$(PRODUCTION_MACOS_CATEGORY)'
	PRODUCTION_MACOS_COPYRIGHT := $(PRODUCTION_MACOS_COPYRIGHT)
...
PCH_COMPILE = $(CC) -x c++-header $(CFLAGS_DEPS) $(_BUILD_MACROS) $(CFLAGS) $(_INCLUDE_DIRS) -o $@ -c $<
ifneq ($(_PCH),)
...
ifeq ($(shell brew ls --versions makeicns),)
	HOMEBREW_NO_AUTO_UPDATE=1 brew install makeicns
	@echo
	$(color_reset)
endif
ifneq ($(shell which makeicns),)
	@echo '   Generating the icon file...'
	$(_Q)makeicns -in env/osx/$(PRODUCTION_MACOS_ICON).png -out $(PRODUCTION_FOLDER)/Resources/$(PRODUCTION_MACOS_ICON).icns
	$(color_reset)
else
	@echo '   Warning: makeicns is not available, could not generate icon file.'
	@echo '   If makeicns just got installed, please run make again.'
endif
	@echo
	...
	$(_Q)plutil -replace CFBundleVersion -string $(PRODUCTION_MACOS_BUNDLE_VERSION) $(PRODUCTION_FOLDER)/Info.plist
	$(_Q)plutil -replace LSApplicationCategoryType -string $(PRODUCTION_MACOS_CATEGORY) $(PRODUCTION_FOLDER)/Info.plist
	$(_Q)plutil -replace NSHumanReadableCopyright -string $(PRODUCTION_MACOS_COPYRIGHT) $(PRODUCTION_FOLDER)/Info.plist
